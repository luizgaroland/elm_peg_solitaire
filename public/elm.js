...
Elm.Game = Elm.Game || {};
Elm.Game.Definition = Elm.Game.Definition || {};
Elm.Game.Definition.make = function (_elm) {
   "use strict";
   _elm.Game = _elm.Game || {};
   _elm.Game.Definition = _elm.Game.Definition || {};
   if (_elm.Game.Definition.values) return _elm.Game.Definition.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var cursorOrigin = {ctor: "_Tuple2",_0: 4,_1: 4};
   var Game = F3(function (a,b,c) {    return {gameState: a,board: b,cursor: c};});
   var Play = F3(function (a,b,c) {    return {makePlayPressed: a,cursorDirection: b,cursorDeviation: c};});
   var Arrow = F2(function (a,b) {    return {x: a,y: b};});
   var BoardCircle = function (a) {    return {hasPiece: a};};
   var Win = {ctor: "Win"};
   var Loss = {ctor: "Loss"};
   var PlayingToChooseDirection = {ctor: "PlayingToChooseDirection"};
   var Playing = {ctor: "Playing"};
   var Origin = {ctor: "Origin"};
   var InvalidDirection = {ctor: "InvalidDirection"};
   var West = {ctor: "West"};
   var East = {ctor: "East"};
   var South = {ctor: "South"};
   var North = {ctor: "North"};
   return _elm.Game.Definition.values = {_op: _op
                                        ,North: North
                                        ,South: South
                                        ,East: East
                                        ,West: West
                                        ...
                                        ,Origin: Origin
                                        ,Playing: Playing
                                        ,PlayingToChooseDirection: PlayingToChooseDirection
                                        ,Loss: Loss
                                        ,Win: Win
...
Elm.Game = Elm.Game || {};
Elm.Game.BoardCircle = Elm.Game.BoardCircle || {};
Elm.Game.BoardCircle.make = function (_elm) {
   "use strict";
   _elm.Game = _elm.Game || {};
   _elm.Game.BoardCircle = _elm.Game.BoardCircle || {};
   if (_elm.Game.BoardCircle.values) return _elm.Game.BoardCircle.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var isBoardCircleOnCoordinateWithPiece = F3(function (coordinate,coordinate$,boardCircle) {
      return _U.eq(coordinate,coordinate$) ? boardCircle.hasPiece ? true : false : false;
   });
   var isBoardCircleOnListAndWithoutPiece = F3(function (coordinates,coordinate,boardCircle) {
      return A2($List.member,coordinate,coordinates) ? $Basics.not(boardCircle.hasPiece) ? true : false : false;
   });
   var isBoardCircleOnListAndWithPiece = F3(function (coordinates,coordinate,boardCircle) {
      return A2($List.member,coordinate,coordinates) ? boardCircle.hasPiece ? true : false : false;
   });
   var setBoardCirclePiece = F3(function (withPiece,coordinate,boardCircle) {    return _U.update(boardCircle,{hasPiece: withPiece});});
   var updateBoardCirclePiece = F2(function (withPiece,boardCircle) {
      var _p0 = boardCircle;
      if (_p0.ctor === "Just") {
            return $Maybe.Just(_U.update(_p0._0,{hasPiece: withPiece}));
         } else {
            return boardCircle;
         }
   });
   var zipCoordWithBoardCircles = F2(function (coords,circles) {
      return A3($List.map2,F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};}),coords,circles);
   });
   var getY = function (tuple) {    return $Basics.snd(tuple);};
   var getX = function (tuple) {    return $Basics.fst(tuple);};
   var boardCircle = function (hpiece) {    return {hasPiece: hpiece};};
   var coordinate = F2(function (x,y) {    return {ctor: "_Tuple2",_0: x,_1: y};});
   var validateCoord = function (coord) {
      return _U.cmp(getX(coord),2) > 0 && _U.cmp(getX(coord),6) < 0 && (_U.cmp(getY(coord),0) > 0 && _U.cmp(getY(coord),4) < 0) ? true : _U.cmp(getX(coord),
      0) > 0 && _U.cmp(getX(coord),8) < 0 && (_U.cmp(getY(coord),2) > 0 && _U.cmp(getY(coord),6) < 0) ? true : _U.cmp(getX(coord),2) > 0 && _U.cmp(getX(coord),
      6) < 0 && (_U.cmp(getY(coord),5) > 0 && _U.cmp(getY(coord),8) < 0) ? true : false;
   };
   return _elm.Game.BoardCircle.values = {_op: _op
                                         ,validateCoord: validateCoord
                                         ,coordinate: coordinate
                                         ,boardCircle: boardCircle
                                         ,getX: getX
                                         ,getY: getY
                                         ,zipCoordWithBoardCircles: zipCoordWithBoardCircles
                                         ,updateBoardCirclePiece: updateBoardCirclePiece
                                         ,setBoardCirclePiece: setBoardCirclePiece
                                         ,isBoardCircleOnListAndWithPiece: isBoardCircleOnListAndWithPiece
                                         ,isBoardCircleOnListAndWithoutPiece: isBoardCircleOnListAndWithoutPiece
                                         ,isBoardCircleOnCoordinateWithPiece: isBoardCircleOnCoordinateWithPiece};
};
Elm.Game = Elm.Game || {};
Elm.Game.Board = Elm.Game.Board || {};
Elm.Game.Board.make = function (_elm) {
   "use strict";
   _elm.Game = _elm.Game || {};
   _elm.Game.Board = _elm.Game.Board || {};
   if (_elm.Game.Board.values) return _elm.Game.Board.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   $Game$BoardCircle = Elm.Game.BoardCircle.make(_elm),
   $Game$Definition = Elm.Game.Definition.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var isBoardCircleAtRow = F3(function (row,coordinate,boardCircle) {    return _U.eq($Game$BoardCircle.getX(coordinate),row) ? true : false;});
   var getCirclesAtRow = F2(function (row,board) {    return A2($Dict.filter,isBoardCircleAtRow(row),board);});
   var setBoardPiece = F3(function (coordinate,withPiece,board) {
      return A3($Dict.update,coordinate,$Game$BoardCircle.updateBoardCirclePiece(withPiece),board);
   });
   var setBoardOrigin = function (board) {
      var board$ = A2($Dict.map,$Game$BoardCircle.setBoardCirclePiece(true),board);
      return A3(setBoardPiece,{ctor: "_Tuple2",_0: 4,_1: 4},false,board$);
   };
   var getBoardCircles = function () {
      var fromDigitToCircle = function (digit) {    return $Game$BoardCircle.boardCircle(false);};
      return A2($List.map,fromDigitToCircle,_U.range(1,33));
   }();
   var addCoordinateOnRow = F2(function (coordinate,num) {
      var y = A2(F2(function (x,y) {    return x + y;}),$Game$BoardCircle.getY(coordinate),num);
      var x = $Game$BoardCircle.getX(coordinate);
      return A2(F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};}),x,y);
   });
   var addCoordinateOnColumn = F2(function (coordinate,num) {
      var y = $Game$BoardCircle.getY(coordinate);
      var x = A2(F2(function (x,y) {    return x + y;}),$Game$BoardCircle.getX(coordinate),num);
      return A2(F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};}),x,y);
   });
   var getCoordinatesRow = function (digit) {    return A2($List.map,$Game$BoardCircle.coordinate(digit),_U.range(1,7));};
   var getCoordinatesList = $List.concat(A2($List.map,getCoordinatesRow,_U.range(1,7)));
   var getValidCoordinates = A2($List.filter,$Game$BoardCircle.validateCoord,getCoordinatesList);
   var createBoard = $Dict.fromList(A2($Game$BoardCircle.zipCoordWithBoardCircles,getValidCoordinates,getBoardCircles));
   return _elm.Game.Board.values = {_op: _op
                                   ,getCoordinatesRow: getCoordinatesRow
                                   ,getCoordinatesList: getCoordinatesList
                                   ,addCoordinateOnColumn: addCoordinateOnColumn
                                   ,addCoordinateOnRow: addCoordinateOnRow
                                   ,getValidCoordinates: getValidCoordinates
                                   ,getBoardCircles: getBoardCircles
                                   ,createBoard: createBoard
                                   ,setBoardPiece: setBoardPiece
                                   ,setBoardOrigin: setBoardOrigin
                                   ,isBoardCircleAtRow: isBoardCircleAtRow
                                   ,getCirclesAtRow: getCirclesAtRow};
};
Elm.Game = Elm.Game || {};
Elm.Game.Controls = Elm.Game.Controls || {};
Elm.Game.Controls.make = function (_elm) {
   "use strict";
   _elm.Game = _elm.Game || {};
   _elm.Game.Controls = _elm.Game.Controls || {};
   if (_elm.Game.Controls.values) return _elm.Game.Controls.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Game$Definition = Elm.Game.Definition.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var makePlayKeyPressed = $Keyboard.space;
   var fromDeviationToDirection = function (cursor) {
      var cursorY = $Basics.snd(cursor);
      var cursorX = $Basics.fst(cursor);
      return _U.eq(cursorX,1) && _U.eq(cursorY,0) ? $Game$Definition.South : _U.eq(cursorX,-1) && _U.eq(cursorY,0) ? $Game$Definition.North : _U.eq(cursorX,
      0) && _U.eq(cursorY,1) ? $Game$Definition.East : _U.eq(cursorX,0) && _U.eq(cursorY,-1) ? $Game$Definition.West : $Game$Definition.InvalidDirection;
   };
   var fromArrowsToCoordinates = F2(function (x,y) {    return A2(F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};}),0 - x.y,y.x);});
   var cursorCoordinateX = $Keyboard.arrows;
   var cursorCoordinateY = $Keyboard.arrows;
   var getCursorDeviation = A3($Signal.map2,fromArrowsToCoordinates,cursorCoordinateX,cursorCoordinateY);
   var getCursorDirection = A2($Signal.map,fromDeviationToDirection,getCursorDeviation);
   return _elm.Game.Controls.values = {_op: _op
                                      ,cursorCoordinateY: cursorCoordinateY
                                      ,cursorCoordinateX: cursorCoordinateX
                                      ,fromArrowsToCoordinates: fromArrowsToCoordinates
                                      ,getCursorDeviation: getCursorDeviation
                                      ,fromDeviationToDirection: fromDeviationToDirection
                                      ,getCursorDirection: getCursorDirection
                                      ,makePlayKeyPressed: makePlayKeyPressed};
};
Elm.Game = Elm.Game || {};
Elm.Game.Logic = Elm.Game.Logic || {};
Elm.Game.Logic.make = function (_elm) {
   "use strict";
   _elm.Game = _elm.Game || {};
   _elm.Game.Logic = _elm.Game.Logic || {};
   if (_elm.Game.Logic.values) return _elm.Game.Logic.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   ...
   var makeDirectionsFromBooleans = F4(function (north,south,east,west) {
      ...
      A2($Basics._op["++"],
      south ? A2($List._op["::"],$Game$Definition.South,_U.list([])) : _U.list([]),
      A2($Basics._op["++"],
      east ? A2($List._op["::"],$Game$Definition.East,_U.list([])) : _U.list([]),
      west ? A2($List._op["::"],$Game$Definition.West,_U.list([])) : _U.list([]))));
   });
   var canPlayHappenWhichDirection = F2(function (coordinate,board) {
      var furtherWest = A2($Game$Board.addCoordinateOnRow,coordinate,-2);
      var furtherWestWithoutPiece = $Basics.not($Dict.isEmpty(A2($Dict.filter,
      $Game$BoardCircle.isBoardCircleOnListAndWithoutPiece(_U.list([furtherWest])),
      board)));
      var furtherEast = A2($Game$Board.addCoordinateOnRow,coordinate,2);
      var furtherEastWithoutPiece = $Basics.not($Dict.isEmpty(A2($Dict.filter,
      $Game$BoardCircle.isBoardCircleOnListAndWithoutPiece(_U.list([furtherEast])),
      board)));
      var furtherSouth = A2($Game$Board.addCoordinateOnColumn,coordinate,2);
      var furtherSouthWithoutPiece = $Basics.not($Dict.isEmpty(A2($Dict.filter,
      $Game$BoardCircle.isBoardCircleOnListAndWithoutPiece(_U.list([furtherSouth])),
      board)));
      var furtherNorth = A2($Game$Board.addCoordinateOnColumn,coordinate,-2);
      var furtherNorthWithoutPiece = $Basics.not($Dict.isEmpty(A2($Dict.filter,
      $Game$BoardCircle.isBoardCircleOnListAndWithoutPiece(_U.list([furtherNorth])),
      board)));
      var west = A2($Game$Board.addCoordinateOnRow,coordinate,-1);
      var westWithPiece = $Basics.not($Dict.isEmpty(A2($Dict.filter,$Game$BoardCircle.isBoardCircleOnListAndWithPiece(_U.list([west])),board)));
      var westCan = westWithPiece && furtherWestWithoutPiece;
      var east = A2($Game$Board.addCoordinateOnRow,coordinate,1);
      var eastWithPiece = $Basics.not($Dict.isEmpty(A2($Dict.filter,$Game$BoardCircle.isBoardCircleOnListAndWithPiece(_U.list([east])),board)));
      var eastCan = eastWithPiece && furtherEastWithoutPiece;
      var south = A2($Game$Board.addCoordinateOnColumn,coordinate,1);
      var southWithPiece = $Basics.not($Dict.isEmpty(A2($Dict.filter,$Game$BoardCircle.isBoardCircleOnListAndWithPiece(_U.list([south])),board)));
      var southCan = southWithPiece && furtherSouthWithoutPiece;
      var north = A2($Game$Board.addCoordinateOnColumn,coordinate,-1);
      var northWithPiece = $Basics.not($Dict.isEmpty(A2($Dict.filter,$Game$BoardCircle.isBoardCircleOnListAndWithPiece(_U.list([north])),board)));
      var northCan = northWithPiece && furtherNorthWithoutPiece;
      var directionList = A4(makeDirectionsFromBooleans,northCan,southCan,eastCan,westCan);
      var atCoordinateHavePiece = $Basics.not($Dict.isEmpty(A2($Dict.filter,$Game$BoardCircle.isBoardCircleOnCoordinateWithPiece(coordinate),board)));
      return atCoordinateHavePiece ? northCan || (southCan || (eastCan || westCan)) ? {ctor: "_Tuple2",_0: true,_1: directionList} : {ctor: "_Tuple2"
                                                                                                                                     ,_0: false
                                                                                                                                     ,_1: directionList} : {ctor: "_Tuple2"
                                                                                                                                                           ,_0: false
                                                                                                                                                           ,_1: directionList};
   });
   var makePlay = F3(function (coordinate,direction,board) {
      var playAndDirection = A2(canPlayHappenWhichDirection,coordinate,board);
      var doMakePlay = $Basics.fst(playAndDirection);
      var directions = $Basics.snd(playAndDirection);
      var validDirection = A2($List.member,direction,directions);
      if (doMakePlay && validDirection) {
            var _p0 = direction;
            switch (_p0.ctor)
            {case "North": var furtherNorth = A2($Game$Board.addCoordinateOnColumn,coordinate,-2);
                 var north = A2($Game$Board.addCoordinateOnColumn,coordinate,-1);
                 return A3($Game$Board.setBoardPiece,
                 coordinate,
                 false,
                 A3($Game$Board.setBoardPiece,furtherNorth,true,A3($Game$Board.setBoardPiece,north,false,board)));
               case "South": var furtherSouth = A2($Game$Board.addCoordinateOnColumn,coordinate,2);
                 var south = A2($Game$Board.addCoordinateOnColumn,coordinate,1);
                 return A3($Game$Board.setBoardPiece,
                 coordinate,
                 false,
                 A3($Game$Board.setBoardPiece,furtherSouth,true,A3($Game$Board.setBoardPiece,south,false,board)));
               case "East": var furtherEast = A2($Game$Board.addCoordinateOnRow,coordinate,2);
                 var east = A2($Game$Board.addCoordinateOnRow,coordinate,1);
                 return A3($Game$Board.setBoardPiece,
                 coordinate,
                 false,
                 A3($Game$Board.setBoardPiece,furtherEast,true,A3($Game$Board.setBoardPiece,east,false,board)));
               case "West": var furtherWest = A2($Game$Board.addCoordinateOnRow,coordinate,-2);
                 var west = A2($Game$Board.addCoordinateOnRow,coordinate,-1);
                 return A3($Game$Board.setBoardPiece,
                 coordinate,
                 false,
                 A3($Game$Board.setBoardPiece,furtherWest,true,A3($Game$Board.setBoardPiece,west,false,board)));
               default: return board;}
         } else return board;
   });
   var isPlayPossible = F3(function (board,coordinate,boardCircle) {    return $Basics.fst(A2(canPlayHappenWhichDirection,coordinate,board));});
   var arePlaysPossible = function (board) {
      var listOfPossiblePlays = $Dict.toList(A2($Dict.filter,isPlayPossible(board),board));
   ...
   return _elm.Game.Logic.values = {_op: _op
                                   ...
                                   ,makeDirectionsFromBooleans: makeDirectionsFromBooleans
...
Elm.Update.make = function (_elm) {
   ...
   $Debug = Elm.Debug.make(_elm),
   $Game$BoardCircle = Elm.Game.BoardCircle.make(_elm),
   $Game$Controls = Elm.Game.Controls.make(_elm),
   $Game$Definition = Elm.Game.Definition.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var fromSignalsToPlayRecord = F3(function (makePlayPressed,deviation,direction) {
      return {makePlayPressed: makePlayPressed,cursorDirection: direction,cursorDeviation: deviation};
   });
   var getPlay = A4($Signal.map3,fromSignalsToPlayRecord,$Game$Controls.makePlayKeyPressed,$Game$Controls.getCursorDeviation,$Game$Controls.getCursorDirection);
   var updateCursor = F3(function (state,cursor,deviation) {
      var oldCursor = A2(F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};}),$Basics.fst(cursor),$Basics.snd(cursor));
   ...
   var updateGame = F2(function (play,game) {
      ...
      var cursorDirection = play.cursorDirection;
      var newCursor = A3(updateCursor,game.gameState,game.cursor,play.cursorDeviation);
      var newGame = _U.update(game,{cursor: newCursor});
      var playAndDirection = A2($Game$Logic.canPlayHappenWhichDirection,newGame.cursor,newGame.board);
      var canPlayHappen = $Basics.fst(playAndDirection);
      var playDirections = $Basics.snd(playAndDirection);
   ...
   return _elm.Update.values = {_op: _op
                               ...
                               ,fromSignalsToPlayRecord: fromSignalsToPlayRecord
                               ,getPlay: getPlay
                               ,getGame: getGame};
};
Elm.View = Elm.View || {};
Elm.View.BoardPeripherals = Elm.View.BoardPeripherals || {};
Elm.View.BoardPeripherals.make = function (_elm) {
   "use strict";
   _elm.View = _elm.View || {};
   _elm.View.BoardPeripherals = _elm.View.BoardPeripherals || {};
   if (_elm.View.BoardPeripherals.values) return _elm.View.BoardPeripherals.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var boardCircleFillerClass = A2($Basics._op["++"],"filler-circle ",boardCircleClass);
   var fillerCircle = A2($Html.i,_U.list([$Html$Attributes.$class(boardCircleFillerClass)]),_U.list([]));
   var renderOuterCircle = A2($Html.i,_U.list([$Html$Attributes.$class(boardCircleClass)]),_U.list([]));
...
Elm.View.Board.make = function (_elm) {
   ...
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $View$BoardPeripherals = Elm.View.BoardPeripherals.make(_elm);
   var _op = {};
   var renderFillerCircle = A2($Html.td,
   _U.list([]),
   _U.list([A2($Html.span,_U.list([$Html$Attributes.$class($View$BoardPeripherals.circleWrapperClass)]),_U.list([$View$BoardPeripherals.fillerCircle]))]));
   var isCoordinateAtCursor = F2(function (coordinate,cursor) {
      var coordinateY = $Basics.snd(coordinate);
      var coordinateX = $Basics.fst(coordinate);
      var cursorY = $Basics.snd(cursor);
      var cursorX = $Basics.fst(cursor);
   ...
   return _elm.View.Board.values = {_op: _op
                                   ...
                                   ,isCoordinateAtCursor: isCoordinateAtCursor
                                   ,renderBoardCircle: renderBoardCircle
                                   ,renderFillerCircle: renderFillerCircle
                                   ,getHtmlCircleAndWrapper: getHtmlCircleAndWrapper
                                   ,getHtmlFromBoardRow: getHtmlFromBoardRow
                                   ,renderTrimmedRow: renderTrimmedRow
                                   ,renderRow: renderRow
                                   ,renderBoardTrimmedRowsTop: renderBoardTrimmedRowsTop
                                   ,renderBoardTrimmedRowsBot: renderBoardTrimmedRowsBot
                                   ,renderBoardCentralCluster: renderBoardCentralCluster
                                   ,renderBoard: renderBoard};
};
Elm.View = Elm.View || {};
Elm.View.Canvas = Elm.View.Canvas || {};
Elm.View.Canvas.make = function (_elm) {
   "use strict";
   _elm.View = _elm.View || {};
   _elm.View.Canvas = _elm.View.Canvas || {};
   if (_elm.View.Canvas.values) return _elm.View.Canvas.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Game$Definition = Elm.Game.Definition.make(_elm),
   ...
   var mainFlowDown = F2(function (_p0,game) {
      ...
      _U.list([A4($Graphics$Element.container,
              ...
              ,A4($Graphics$Element.container,_p2,325,$Graphics$Element.middle,A3($Html.toElement,335,325,$View$Board.renderBoard(game)))
              ,A4($Graphics$Element.container,
              _p2,
              80,
              $Graphics$Element.middle,
              A3($Html.toElement,90,50,A2($Html.h2,_U.list([]),_U.list([$Html.text("Controls")]))))
              ,A4($Graphics$Element.container,
              _p2,
              50,
              $Graphics$Element.middle,
              A3($Html.toElement,121,50,A2($Html.p,_U.list([]),_U.list([$Html.text("Space - Make Play")]))))
              ,A4($Graphics$Element.container,
              _p2,
              50,
              $Graphics$Element.middle,
              ...
              303,
              50,
              A2($Html.p,_U.list([]),_U.list([$Html.text("Arrows - Move Cursor | Choose Play Direction")]))))])) : _U.eq(game.gameState,
      ...
      _U.list([A4($Graphics$Element.container,
              ...
              150,
              $Graphics$Element.middle,
              A3($Html.toElement,185,80,A2($Html.h4,_U.list([]),_U.list([$Html.text("Press Space to Play Again!")]))))
              ,A4($Graphics$Element.container,
              _p2,
              80,
              $Graphics$Element.middle,
      ...
      _U.list([A4($Graphics$Element.container,
              ...
              303,
              50,
              A2($Html.p,_U.list([]),_U.list([$Html.text("Arrows - Move Cursor | Choose Play Direction")]))))])) : A2($Graphics$Element.flow,
      $Graphics$Element.down,
      _U.list([A4($Graphics$Element.container,
      _p2,
      100,
