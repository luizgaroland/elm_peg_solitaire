...
Elm.Game.Board.make = function (_elm) {
   ...
   var _op = {};
   var fromTupleToCoordinate = function (tuple) {    return {x: $Basics.fst(tuple),y: $Basics.snd(tuple)};};
   var getTuple = F2(function (x,y) {    return {ctor: "_Tuple2",_0: x,_1: y};});
   var getTuplesRow = function (digit) {    return A2($List.map,getTuple(digit),_U.range(1,7));};
   var getTuplesList = $List.concat(A2($List.map,getTuplesRow,_U.range(1,7)));
   var validateCoord = function (coord) {
      return _U.cmp(coord.x,2) > 0 && _U.cmp(coord.x,6) < 0 && (_U.cmp(coord.y,0) > 0 && _U.cmp(coord.y,4) < 0) ? true : _U.cmp(coord.x,
      0) > 0 && _U.cmp(coord.x,8) < 0 && (_U.cmp(coord.y,2) > 0 && _U.cmp(coord.y,6) < 0) ? true : _U.cmp(coord.x,2) > 0 && _U.cmp(coord.x,
      6) < 0 && (_U.cmp(coord.y,5) > 0 && _U.cmp(coord.y,8) < 0) ? true : false;
   };
   var coordinate = F2(function (x,y) {    return {x: x,y: y};});
   var BoardCircle = F2(function (a,b) {    return {hasPiece: a,coord: b};});
   ...
   return _elm.Game.Board.values = {_op: _op
                                   ...
                                   ,getTuple: getTuple
                                   ,getTuplesRow: getTuplesRow
                                   ,getTuplesList: getTuplesList
                                   ,fromTupleToCoordinate: fromTupleToCoordinate};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var main = $Graphics$Element.show($List.length(A2($List.filter,
   $Game$Board.validateCoord,
   A2($List.map,$Game$Board.fromTupleToCoordinate,$Game$Board.getTuplesList))));
